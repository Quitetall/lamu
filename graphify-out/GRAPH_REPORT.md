...
## Corpus Check
- 147 files · ~79,057 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1968 nodes · 4751 edges · 153 communities detected
- Extraction: 44% EXTRACTED · 56% INFERRED · 0% AMBIGUOUS · INFERRED: 2645 edges (avg confidence: 0.58)
...
Cohesion: 0.04
Nodes (242): ChatRequest, Message, OpenAI-compatible HTTP API layer.  Translates /v1/chat/completions → internal ro, Start the OpenAI-compat server., Start the OpenAI-compat server., Start the OpenAI-compat server., Start the OpenAI-compat server., Create the OpenAI-compatible FastAPI app. (+234 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (149): make_backend(), write_registry(), test_get_extractor_picks_correct_type(), test_write_then_load_roundtrip(), Corrupt YAML registry → load_registry raises (no silent fallback)., test_corrupt_yaml_raises(), test_missing_required_field_raises(), test_unknown_capability_raises() (+141 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (60): ABC, Backend, Backend, Backend protocol — interface that all model backends implement., Abstract base for model backends.      Each backend manages one model process (o, LlamaCppBackend, llama.cpp backend — manages llama-server subprocess., Backend for llama-server (llama.cpp HTTP server). (+52 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (85): get_config(), Return a LangChain RunnableConfig dict with active callbacks., compare(), load_swebench_tasks(), main(), Agentic benchmark runner — compare Opus solo vs swarm on coding tasks.  Three mo, Load SWE-bench Lite tasks. Requires swebench package., Run a task with just Opus (cloud-only, no local workers). (+77 more)

### Community 4 - "Community 4"
Cohesion: 0.06
Nodes (87): Malformed GGUF — emitted via warnings.warn during scan., Think-block streaming buffer exceeded the configured cap (64 KB)., ReasoningOverflow, RegistryParseWarning, get_extractor(), NullReasoningExtractor, Reasoning extractor — per-model-family think-block detection and stripping., Handles think-block detection, stripping, and structured extraction.      Regist (+79 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (52): sched(), test_health_state_gauge_uses_numeric_encoding(), test_queue_depth_gauge_per_model(), test_render_contains_lamu_metrics(), test_vram_total_gauge_reflects_scheduler(), RestartPolicy, test_dead_threshold_at_three(), test_first_error_promotes_to_degraded() (+44 more)
...
Cohesion: 0.06
Nodes (35): LamuMetrics, Prometheus metrics for the OpenAI-compat layer.  All metrics live on a process-l, Pull instantaneous values into the gauges. Call from /metrics., Serialise to Prometheus text. Returns `(body, content_type)`., Bundle of collectors keyed to a single registry. The OpenAI compat     app holds, create_app(), serve(), metrics() (+27 more)

### Community 12 - "Community 12"
Cohesion: 0.05
Nodes (29): app(), _fake_resp(), Tests for lamu.api.openai_compat — FastAPI / OpenAI shim., test_chat_completions_502_on_backend_unreachable(), test_chat_completions_extracts_reasoning_when_present(), test_chat_completions_routes_to_loaded(), marker(), test_loaded_model_mutable() (+21 more)

### Community 13 - "Community 13"
Cohesion: 0.07
Nodes (7): DflashBackend, MegakernelBackend, Backend, ChatMessage, chat_completions(), backend_format(), Error
...
Cohesion: 0.1
Nodes (22): auto_start(), cmd_models(), cmd_status(), discover_models(), get_available_models(), main(), probe_endpoint(), Return flat list of available model names from all endpoints. (+14 more)
...
Cohesion: 0.14
Nodes (17): filter_think(), filter_think_carries_state_across_chunks(), filter_think_pass_through_when_show_enabled(), filter_think_within_single_chunk(), handle_command(), http_get_json(), Message, parse_command() (+9 more)

### Community 22 - "Community 22"
Cohesion: 0.14
Nodes (15): add_via_wizard(), CloudModel, CloudModelList, default_seed(), dummy(), full_id_honours_explicit_model_id(), key_absent_when_env_unset(), key_present_when_no_env_required() (+7 more)
...
Cohesion: 0.27
Nodes (6): initialize_response(), initialize_response_shape(), ServerState, tools_list_response(), tools_list_response_includes_all_tools(), tools_list_response_query_requires_prompt()
...
## Knowledge Gaps
- **327 isolated node(s):** `OpenAI-compatible HTTP server on top of test_dflash.      pip install fastapi uv`, `Infer the HuggingFace tokenizer repo from a GGUF target file.      The GGUF file`, `Extract <tool_call>...</tool_call> blocks from generated text.     Handles two f`, `SGLang launcher with qwen35 GGUF patch. Applies the monkey-patch then starts SGL`, `Monkey-patch transformers to support qwen35 GGUF architecture.  Qwen3.5/3.6 uses` (+322 more)
...
- **Why does `VramScheduler` connect `Community 0` to `Community 2`, `Community 5`, `Community 6`, `Community 11`, `Community 12`?**
  _High betweenness centrality (0.082) - this node is a cross-community bridge._
- **Why does `BackendError` connect `Community 0` to `Community 2`, `Community 5`, `Community 6`, `Community 8`, `Community 14`, `Community 15`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `ModelEntry` connect `Community 0` to `Community 2`, `Community 4`, `Community 6`?**
