...
fn schema_review_commit() -> Value {
    json!({
        ...
        "properties": {
            ...
            "preset": {"type": "string", "enum": ["fast", "max"], "default": "fast", "description": "Quality/cost preset. 'fast' (~$0.003/review): cache-stable single Pro pass with the central FP-list, activity-log cache lock, and structural cross-ref marker. 2-3 findings typical. Use for routine commit gating + every-commit policy. 'max' (~$0.013/review, ~4x cost): adds critic-role parallel pass (LAMU_CRITIC_PASS), multi-model ensemble (LAMU_ENSEMBLE_REVIEW — uses claude-opus-4-7 if ANTHROPIC_API_KEY is set, else falls back to v4-flash), and test pre-flight (LAMU_TEST_PREFLIGHT). 5-7 findings with cross-provider diversity. Use for pre-merge / pre-release / security-sensitive commits. Per-call arg overrides $LAMU_PRESET env."}
        }
    })
}
...
fn schema_review_diff() -> Value {
    json!({
        ...
        "properties": {
            ...
            "plan_file": {"type": "string", "description": "Optional path to a plan/spec markdown file to inject as Plan-tier context. Overrides $LAMU_PLAN and any auto-detected ~/.claude/plans/active.md."},
            "preset": {"type": "string", "enum": ["fast", "max"], "default": "fast", "description": "Quality/cost preset. Same semantics as review_commit: 'fast' = single Pro pass, 'max' = critic + ensemble + test-preflight. Per-call arg overrides $LAMU_PRESET env."}
        },
        "required": ["diff"]
    })
}
