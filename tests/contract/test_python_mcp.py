"""Smoke contract: Python MCP server replays the corpus and produces a
well-formed response per request."""
from __future__ import annotations

import pytest

from tests.contract.conftest import normalise, replay


@pytest.mark.contract
def test_python_corpus_runs(python_mcp_cmd, corpus):
    responses = replay(python_mcp_cmd, corpus, timeout=20)
    # Notifications produce no response, so expect (n_requests - n_notifications).
    expected = sum(1 for r in corpus if "id" in r)
    assert len(responses) == expected, (
        f"got {len(responses)} responses for {expected} id'd requests; "
        f"raw responses: {responses}"
    )

    # Each response is well-formed JSON-RPC.
    for r in responses:
        assert r["jsonrpc"] == "2.0"
        assert "id" in r
        assert "result" in r or "error" in r


@pytest.mark.contract
def test_python_initialize_advertises_tools(python_mcp_cmd, corpus):
    responses = replay(python_mcp_cmd, corpus, timeout=20)
    init_resp = next(r for r in responses if r["id"] == 1)
    norm = normalise(init_resp)
    assert norm["jsonrpc"] == "2.0"
    caps = norm["result"]["capabilities"]
    assert "tools" in caps


@pytest.mark.contract
def test_python_tools_list_includes_query(python_mcp_cmd, corpus):
    responses = replay(python_mcp_cmd, corpus, timeout=20)
    list_resp = next(r for r in responses if r["id"] == 2)
    names = {t["name"] for t in list_resp["result"]["tools"]}
    # The 8 stable tools — adding a tool means updating the corpus.
    assert {
        "query", "plan_query", "list_models", "load_model",
        "unload_model", "vram_status", "scan_models", "queue_status",
    }.issubset(names)
