...
class ChatMessage(BaseModel):
    ...
    tool_calls: list[dict] | None = None   # assistant → tool call
    tool_call_id: str | None = None        # tool result message
    name: str | None = None               # tool result message
...
def _parse_tool_calls(text: str) -> tuple[str, list]:
    ...
    Handles two formats emitted by Qwen3.5:
      1. JSON:     <tool_call>{"name":"f","arguments":{...}}</tool_call>
      2. Function: <tool_call><function=f><parameter=p>v</parameter>...</function></tool_call>
    Returns (text_with_blocks_removed, list_of_openai_tool_call_dicts).
    """
    tool_calls = []
    ...
    for match in re.finditer(pattern, text, re.DOTALL):
        ...
        try:
            ...
            pass
        # Try <function=name><parameter=p>v</parameter>...</function> format
        if call is None:
            func_m = re.search(r"<function=(\w+)>(.*?)</function>", inner, re.DOTALL)
            if func_m:
                name = func_m.group(1)
                params: dict = {}
                for pm in re.finditer(r"<parameter=(\w+)>(.*?)</parameter>", func_m.group(2), re.DOTALL):
    ...
    def _tokenize_prompt(req: ChatRequest) -> Path:
        ...
        for m in req.messages:
            ...
            if m.content is not None:
                msg["content"] = (
                    _anthropic_text_from_content(m.content)
                    if isinstance(m.content, list)
                    else m.content
                )
            if m.tool_calls:
                # Qwen3.5 template expects arguments as a dict, not a JSON string
                fixed = []
